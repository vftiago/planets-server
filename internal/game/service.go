...
func (s *Service) generateUniverse(gameID int, config UniverseConfig) error {
	...
	galaxies, err := s.spatialService.GenerateEntities(&gameID, spatial.EntityTypeGalaxy, config.GalaxyCount)
	if err != nil {
	...
	for _, galaxy := range galaxies {
		galaxySectors, err := s.spatialService.GenerateEntities(&galaxy.ID, spatial.EntityTypeSector, config.SectorsPerGalaxy)
		if err != nil {
	...
	for _, sector := range sectors {
		sectorSystems, err := s.spatialService.GenerateEntities(&sector.ID, spatial.EntityTypeSystem, config.SystemsPerSector)
		if err != nil {
