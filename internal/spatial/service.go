...
func (s *Service) CreateEntity(parentID *int, entityType EntityType, x, y int, name string) (*SpatialEntity, error) {
	level := EntityLevels[entityType]
	return s.repo.CreateEntity(parentID, entityType, level, x, y, name, "")
}
...
func (s *Service) GenerateEntities(parentID *int, entityType EntityType, count int) ([]SpatialEntity, error) {
	logger := s.logger.With("operation", "generate_entities", "type", entityType, "count", count)
	...
	for x := 0; x < entitiesPerSide; x++ {
		for y := 0; y < entitiesPerSide; y++ {
			...
			entity, err := s.CreateEntity(parentID, entityType, x, y, name)
			if err != nil {
